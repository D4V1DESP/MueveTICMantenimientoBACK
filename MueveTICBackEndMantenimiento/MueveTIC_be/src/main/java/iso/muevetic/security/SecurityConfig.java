    ...
    SecurityFilterChain filterChain(HttpSecurity http, 	@Autowired JWTAuthorizationFilter authFilter) throws Exception {
    	return http
        	...
        	.authorizeHttpRequests()
    			...
    			.hasAnyAuthority(UserRole.MEMBER.name(), UserRole.ADMIN.name(), UserRole.MAINTENANCE.name())
    			.requestMatchers(HttpMethod.PATCH, "/me/")
    			.hasAnyAuthority(UserRole.MEMBER.name(), UserRole.ADMIN.name())
    		...
    		.authorizeHttpRequests()
    			.requestMatchers("/me/**")
    			.hasAuthority(UserRole.MEMBER.name())
    			.and()
